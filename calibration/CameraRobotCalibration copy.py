...
markerLength = 0.0045
markerSeparation = 0.002
#self.aruco_dict = cv2.aruco.Dictionary_get( cv2.aruco.DICT_6X6_1000 )
aruco_dict = cv2.aruco.Dictionary_get( cv2.aruco.DICT_4X4_1000 )

board = cv2.aruco.GridBoard_create(2, 2, markerLength, markerSeparation, aruco_dict) 
img = cv2.aruco.drawPlanarBoard(board, (100,100))# for printing on A4 paper
cv2.imwrite('/home/ruthrash/test.jpg', img)          
        '''
    ...
    def WaitAndCollectData(self):
        ...
        if(self.args.camera_in_hand):#camera in hand
            ...
            cam2gripper_pose[0, -1] = t_cam2gripper[0,0] ; cam2gripper_pose[1, -1] = t_cam2gripper[0,1]; cam2gripper_pose[2, -1] = t_cam2gripper[0,2]
            inv = tf_utils.inverse_matrix(cam2gripper_pose)
            print(inv)            
    ...
    #         cam2gripper_pose = np.eye(4)
    #         cam2gripper_pose[0:3, 0:3] = R_cam2gripper
    #         cam2gripper_pose[0:3, -1] = t_cam2gripper[:,0]
    #         inv = tf_utils.inverse_matrix(cam2gripper_pose)
    #         print(inv)            
        # else:# camera in environment
        #     retval, cameraMatrix, distCoeffs, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors =cv2.aruco.calibrateCameraArucoExtended(all_corners,
...
def main(args):
    ...
    print(args.move_robot_automatically)
    #fa.reset_joints()
