...
def create_and_save_arucotag(output_image_file, dictionary, id_ = 0):
    ...
    Args:
        output_image_file (str): path to the file in which the image is to be saved (*.jpg or *.png)
        dictionary (str): ArucoDictionary of the marker  
        id_ (int, optional): ID of the marker to be rendered. Defaults to 0.

    Returns:
        bool: If saving the image with arucotag pattern was successful or not
    
    Raises: 
        Exception: KeyError if the requested dictionary doesn't exist in available dictionaries of aruco markers.KeyError
    """
    try:
...
def create_and_save_arucoboard(output_image_file, dictionary, n_rows, n_cols, marker_length, marker_separation, ids = None):
    ...
    Args:
        output_image_file (str): path to the file in which the image is to be saved (*.jpg or *.png)
        dictionary (str): ArucoDictionary of the markers in the aruco board
