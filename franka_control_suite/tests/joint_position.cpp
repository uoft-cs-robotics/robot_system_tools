...
int main(int argc, char** argv) {
  ...
  //JointPosController controller(filter_size, K_P, K_D);
  JointPosition controller(1);
  std::string file_name = "/home/pairlab/log.txt";  
  bool log_joint_space=true;
  std::thread print_thread = std::thread(log_data, std::cref(print_rate), std::ref(print_data), std::ref(running), std::ref(file_name), std::cref(log_joint_space));
  try {
    ...
    robot.control(
        ...
        franka::JointPositions output = test_joint_pos_motion_generator(robot_state, period, initial_position, index);
        if (index >= 5.0) {
