...
def plot_ee_impedance(imps_cmd, show_plot=True):
    fig, axs = plt.subplots(3,1)    
    fig.suptitle('Vertically stacked cartesian impedance subplots')
    for i in range(3):
        x_axis = []
        y_axis = []
        count = 0 
        for imp_cmd in imps_cmd:
            y_axis.append(imp_cmd[i])
            x_axis.append(count)
            count += 1
        axs[i].plot(x_axis, y_axis,color='r', label='Kp')
        axs[i].set_title('Cartesian Impedance')     
    plt.legend()
    plt.show() 
class SynchMessages:
    def __init__(self,
