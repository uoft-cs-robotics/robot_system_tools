...
if __name__ == "__main__":
    ...
    T = test_config["dt"] * len(waypoints)
    ts = np.arange(0, T, test_config["dt"])
    ...
    fa.goto_pose(p0, duration = T, buffer_time=10,  dynamic = True)
    ...
    for cmd, robot_state in zip(cmds, robot_states):
        ...
        deserialized_impedance_msg = CartesianImpedanceSensorMessage.FromString(cmd.feedbackControllerSensorData.sensorData)
        poses_cmd.append(deserialized_msg.position) 
        trans_impedances_cmd.append(deserialized_impedance_msg.translational_stiffnesses)
        poses_measured.append(np.array([robot_state.O_T_EE[12],
                                robot_state.O_T_EE[13],
                                robot_state.O_T_EE[14]]))
    plot_ee_level(poses_measured, poses_cmd)
    plot_ee_impedance(trans_impedances_cmd)
