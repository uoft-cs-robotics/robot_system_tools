...
if __name__ == "__main__":
    ...
    fa.goto_joints(joints_0, duration=T, dynamic=True, buffer_time=10, use_impedance=True, block=False)
    ...
    for i in range(2, len(ts)):
        ...
        fb_ctrlr_proto = CartesianImpedanceSensorMessage(
            id=i, timestamp=rospy.Time.now().to_time() - init_time,
            translational_stiffnesses=[1000, 1000, 50],
    ...
    for cmd, measured_state in zip(cmds_q, robot_qs):
        deserialized_traj_msg = JointPositionSensorMessage.FromString(cmd.trajectoryGeneratorSensorData.sensorData)
        deserialized_impedance_msg = CartesianImpedanceSensorMessage.FromString(cmd.feedbackControllerSensor_msg.sensorData)
        print(deserialized_impedance_msg)
        # print(deserialized_traj_msg.joints[3])
        qs_cmd.append(deserialized_traj_msg.joints)
